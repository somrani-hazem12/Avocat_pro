...
export class DocumentService {
  ...
  getDownloadUrl(id: number): string {
    ...
    // LA FONCTION IA
 summarize(docId: number): Observable<string> {
  // 1. Récupérer le token stocké lors de la connexion
  const token = localStorage.getItem('token'); 

  // 2. Créer les headers avec le token
  const headers = {
    'Authorization': `Bearer ${token}`
  };

  // 3. Envoyer la requête avec les headers
  return this.http.post(
    `http://localhost:8080/api/chatbot/summarize/${docId}`, 
    {}, 
    { 
      headers: headers, // ON AJOUTE LES HEADERS ICI
      responseType: 'text' 
    }
  );
}
